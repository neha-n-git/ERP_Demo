...
const sendEmail = async ({ to, subject, html, text }) => {
  const from = process.env.SMTP_FROM || '"SJRI Incubation" <no-reply@sjhif.in>';
  if (transporter) {
    try {
...
const sendRegistrationAck = async (startup) => {
  return sendEmail({
    ...
    subject: 'Application Received — SJRI Incubation Centre',
    text: `Dear ${startup.contact_name},\n\nThank you for submitting your application for ${startup.name} to the SJRI Incubation Centre.\nYour application is currently under review. Our team will get back to you shortly.\n\nBest regards,\nSJRI Incubation Centre\nSt. John\'s Health Innovation Foundation`,
    html: `<div style="font-family:Inter,sans-serif;max-width:600px;margin:0 auto"><div style="background:#0D2C54;padding:24px;text-align:center"><img src="https://www.sjhif.in/wp-content/uploads/2019/07/New-SJHIF-logo-July19.png" alt="SJHIF" style="height:50px"/></div><div style="padding:32px;background:#fff"><h2 style="color:#0D2C54">Application Received</h2><p>Dear <b>${startup.contact_name}</b>,</p><p>Thank you for submitting your application for <b>${startup.name}</b>.</p><p>We have received your application and it is currently under review.</p><div style="margin-top:24px;padding:16px;background:#f0fdf4;border-left:4px solid #10B981;border-radius:4px"><b>Status:</b> Pending Review</div></div><div style="padding:16px;background:#f2f3f5;text-align:center;font-size:12px;color:#666">SJRI Incubation Centre | Koramangala, Bangalore</div></div>`,
  });
};
